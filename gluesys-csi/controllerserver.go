...
type controllerServer struct {
	*csicommon.DefaultControllerServer
	...
	csiNodes []util.CSINode // all spdk nodes in cluster
...
func publishVolume(volume *volume) (map[string]string, error) {
	...
	*/
	volumeInfo, err := volume.csiNode.VolumeInfo(volume.csiVolume.GetVolumeId())
	if err != nil {
...
func deleteVolume(volume *volume) error {
	return volume.csiNode.DeleteVolume(volume.csiVolume.GetVolumeId())
}
	...
	for i := range config.Nodes {
		...
		for j := range secret.Tokens {
			...
			if token.Name == node.Name {
				tokenFound = true
				csiNode, err := util.NewCSINode(node.URL, token.UserName, token.Password, node.TargetType, node.TargetAddr)
				if err != nil {
					...
					server.csiNodes = append(server.csiNodes, csiNode)
				}
				break
			}
		}
